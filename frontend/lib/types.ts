...
export async function createEventAPI(
  data: Omit<MeetEvent, "id" | "createdAt">
): Promise<MeetEvent> {
...
export async function joinEventAPI(eventId: string, username: string): Promise<void> {
  try {
    await fetch(`${BACKEND_URL}/api/events/${eventId}/participants`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username }),
    })
  } catch (err) {
    console.error("Failed to register participant in DB:", err)
  }
}

export async function loadEventAPI(id: string): Promise<MeetEvent | null> {
  try {
